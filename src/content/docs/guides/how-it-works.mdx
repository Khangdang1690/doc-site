...
store you picked. The embedded copies pull those WAL frames continuously
so each app instance keeps a fresh local replica.
...
flowchart LR
    ...
    subgraph appB["App instance B"]
        clientB["libSQL client"]
        embedB[("Embedded SQLite<br/>local.db")]
        clientB -- "reads (µs)" --> embedB
    end
    ...
    clientB -- "writes (Hrana / HTTPS)" --> master
    master -- "WAL frames (gRPC sync)" --> clientA
    master -- "WAL frames (gRPC sync)" --> clientB
...
Embedded replicas pull WAL frames from the master's gRPC stream
(port 5001 in the component diagram below), either on an explicit
`sync()` call or on a configurable `syncInterval`. Because both ends
speak the same on-disk format, the frames are applied byte-level to the
local SQLite file — no SQL replay, no logical-to-physical translation,
no schema drift between replicas. Cross-instance latency is the sync
interval plus the gRPC round-trip, typically a few hundred milliseconds.
